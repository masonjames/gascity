...
func TestRepairStrandedPoolWorkerBead_ReopensAfterConfirmationWindow(t *testing.T) {
	...
	repaired := repairStrandedPoolWorkerBead(store, nil, nil, seedSessionInfo(session), "worker", &clock.Fake{Time: now}, &stderr)
	if !repaired {
...
func TestRepairStrandedPoolWorkerBead_DefersInsideConfirmationWindow(t *testing.T) {
	...
	var stderr bytes.Buffer
	if repairStrandedPoolWorkerBead(store, nil, nil, seedSessionInfo(session), "worker", &clock.Fake{Time: now}, &stderr) {
