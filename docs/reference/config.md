...
| `project_hooks` | string |  |  | ProjectHooks controls whether sessions inherit project-scoped provider hooks discovered from their filesystem context. Empty is equivalent to "inherit"; "forbid" requires a pre-start isolation fence. Enum: `inherit`, `forbid` |
...
| `project_hooks` | string |  |  | ProjectHooks overrides the project-scoped hook discovery policy. Enum: `inherit`, `forbid` |
